...
import type { ReadinessStatus, YearReadiness } from '@/data/questionBank'
...
export function YearGrid({ years, playlists, readiness }: YearGridProps) {
  ...
  useEffect(() => {
    ...
    win.showTags = () => {
      setShowTags(true)
      console.log(
        '%c[GSAT English] Readiness tags enabled',
    ...
    win.hideTags = () => {
      setShowTags(false)
      console.log(
        '%c[GSAT English] Readiness tags hidden',
